...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Installs the dependencies needed to test a Boost library."
    )

    parser.add_argument(
        "-v", "--verbose", help="enable verbose output", action="count", default=0
    )
    parser.add_argument(
        "-q", "--quiet", help="quiet output (opposite of -v)", action="count", default=0
    )
    parser.add_argument(
        "-X",
        "--exclude",
    ...
    parser.add_argument(
        "-N",
        "--ignore",
        help="exclude top-level dependency even when found in scan; can be repeated",
        metavar="LIB",
        action="append",
        default=[],
    )
    parser.add_argument(
        "-I",
        "--include",
        help="additional subdirectory to scan; can be repeated",
        metavar="DIR",
        action="append",
        default=[],
    )
    parser.add_argument(
        "-g",
        "--git_args",
        help="additional arguments to `git submodule update`",
        default="",
        action="store",
    )
    parser.add_argument(
        "-u", "--update", help="update <library> before scanning", action="store_true"
    )
    parser.add_argument(
        "library", help="name of library to scan ('libs/' will be prepended)"
    )
